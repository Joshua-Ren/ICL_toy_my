...
Here define a series of samples that can sample different tasks.
For each sampler, we can sample a set of tasks in the same group with different 
parameters.
...
if __name__ == "__main__":
    ...
    import samplers
    task_sampler = get_task_sampler('sparse_linear_regression',n_dims=5, batch_size=4,sparsity=1)
    task_eg = task_sampler()
    ...
    bias = None
    data_sampler = samplers.get_data_sampler('gaussian', n_dims=5,scale=skew)
    xs = data_sampler.sample_xs(n_points=100, b_size=4)
    ys = task_eg.evaluate(xs)
    """
    xs = xs.reshape(-1,2)
